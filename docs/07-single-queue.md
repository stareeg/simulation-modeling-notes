...
Привет! Сегодня мы разберёмся с аналитическим подходом к теории массового обслуживания — будем считать характеристики очередей **по формулам**, без единой симуляции. Конспект написан по материалам курса «Имитационное моделирование» (НИУ ВШЭ). Если ты когда-нибудь задавался вопросом «а можно ли посчитать среднее время ожидания на бумажке?» — можно, и сейчас мы этому научимся.

---

## Зачем это нужно

Весь этот раздел — это **аналитический тулкит** для оценки производительности систем обслуживания. Вместо того чтобы запускать дискретно-событийную симуляцию и ждать, пока она сойдётся, мы получаем точные формулы.

Три ключевые модели, которые мы разберём:
...
- Переходы возможны только на ±1 (никаких «прыжков» через состояния).
...
- Время обслуживания — экспоненциальное с параметром $\mu$ (среднее время обслуживания $1/\mu$).
- Один сервер.
...
Вот все 26 ключевых результатов, собранных в одну таблицу. Рекомендую распечатать и повесить на стену.

| # | Метрика | Формула | Описание |
|---|---|---|---|
| 1 | Трафик-интенсивность | $\rho = \lambda / \mu$ | Доля времени, когда сервер занят |
...
| 5 | Среднее число заявок в системе | $E[n] = \dfrac{\rho}{1-\rho}$ | Включает и очередь, и обслуживание |
| 6 | Дисперсия числа заявок | $Var[n] = \dfrac{\rho}{(1-\rho)^2}$ | Большой разброс при $\rho \to 1$ |
| 7 | Среднее число в очереди | $E[n_q] = \dfrac{\rho^2}{1-\rho}$ | Только ждущие (без обслуживаемых) |
| 8 | Среднее число на сервере | $E[n_s] = \rho$ | Равно загрузке сервера |
| 9 | Среднее время отклика | $E[r] = \dfrac{1/\mu}{1-\rho}$ | Полное время в системе (ожидание + обслуживание) |
| 10 | Среднее время ожидания | $E[w] = \dfrac{\rho/\mu}{1-\rho}$ | Только время в очереди |
| 11 | Среднее время обслуживания | $E[s] = 1/\mu$ | По условию — экспоненциальное |
...
| 19 | P(≥ $k$ заявок) | $\rho^k$ | Вероятность хвоста |
| 20 | P(> $k$ заявок) | $\rho^{k+1}$ | Строго больше $k$ |
| 21 | Дисперсия времени отклика | $Var[r] = \dfrac{1}{\mu^2(1-\rho)^2}$ | Растёт квадратично при $\rho \to 1$ |
| 22 | Дисперсия времени ожидания | $Var[w] = \dfrac{\rho(2-\rho)}{\mu^2(1-\rho)^2}$ | Большой разброс при загрузке |
| 23 | Распределение времени отклика | $F_r(t) = 1 - e^{-\mu(1-\rho)t}$ | Экспоненциальное с параметром $\mu(1-\rho)$ |
| 24 | Квантиль времени отклика | $r_p = \dfrac{-\ln(1-p)}{\mu(1-\rho)}$ | $p$-й перцентиль |
| 25 | Коэфф. вариации $n$ | $C_n = \dfrac{1}{\sqrt{\rho}}$ | Относительный разброс |
| 26 | Остаточное время обслуживания | $E[s_e] = 1/\mu$ | Для экспоненциального — равно среднему (memoryless!) |
...
Обрати внимание: при $\rho \to 1$ практически все метрики уходят в бесконечность. Это не баг, а фича — система на пределе работает крайне нестабильно.
...
- Трафик-интенсивность: $\rho = 125/500 = 0.25$
...
А теперь практический вопрос: **сколько буферов нужно**, чтобы вероятность потери пакета была пренебрежимо мала?
...
!!! tip "Сколько буферов для потерь < $10^{-6}$?"
    Нам нужно $0.25^{B+1} < 10^{-6}$, то есть $(B+1) \cdot \ln(0.25) < -6 \cdot \ln(10)$.
...
Теперь усложняем: вместо одного сервера у нас $m$ параллельных одинаковых серверов. Заявка занимает первый свободный сервер; если все заняты — встаёт в общую очередь.

### Параметры

- $m$ серверов, каждый с интенсивностью обслуживания $\mu$
- Суммарная интенсивность обслуживания (когда все заняты): $m\mu$
- Трафик-интенсивность **на один сервер**: $\rho = \dfrac{\lambda}{m\mu}$ (должна быть $< 1$)
- Суммарная нагрузка: $a = \lambda / \mu = m\rho$
...
Обрати внимание: пока $n \leq m$, интенсивность обслуживания растёт (подключаются новые серверы: $\mu_n = n\mu$). Когда $n > m$, все серверы заняты и $\mu_n = m\mu$.
...
Вероятность того, что все серверы заняты и заявка будет ждать в очереди:

$$
C(m, a) = \frac{\dfrac{a^m}{m!} \cdot \dfrac{1}{1-\rho}}{\displaystyle\sum_{k=0}^{m-1} \frac{a^k}{k!} + \frac{a^m}{m!} \cdot \frac{1}{1-\rho}}
$$

где $a = m\rho = \lambda/\mu$.

!!! note "Формула Эрланга C"
    Эта формула названа в честь **Агнера Краруп Эрланга** — датского инженера, который в начале XX века разработал математическую теорию телефонных сетей. Его работы — фундамент всей теории массового обслуживания.
...
|---|---|---|
| Серверы | 1 | $m$ |
| Загрузка | $\rho = \lambda/\mu$ | $\rho = \lambda/(m\mu)$ |
| Условие стабильности | $\rho < 1$ | $\rho < 1$ |
| $p_0$ | $1 - \rho$ | $\left[\sum_{k=0}^{m-1}\frac{a^k}{k!} + \frac{a^m}{m!(1-\rho)}\right]^{-1}$ |
| $E[n_q]$ | $\dfrac{\rho^2}{1-\rho}$ | $\dfrac{\rho \cdot C(m,a)}{1-\rho}$ |
| $E[w]$ | $\dfrac{\rho/\mu}{1-\rho}$ | $\dfrac{C(m,a)}{m\mu(1-\rho)}$ |
| $E[r]$ | $\dfrac{1/\mu}{1-\rho}$ | $E[w] + 1/\mu$ |
| $E[n]$ | $\dfrac{\rho}{1-\rho}$ | $E[n_q] + \lambda/\mu$ |

!!! info "Эффект масштаба"
    Один сервер с производительностью $m\mu$ **всегда лучше**, чем $m$ серверов с производительностью $\mu$ каждый (при одинаковой суммарной мощности). Это связано с тем, что при нескольких серверах возможна ситуация, когда один сервер простаивает, а перед другим — очередь. Единый быстрый сервер такого не допускает.

---

## Очередь M/M/m/B

В реальных системах буфер конечный — будь то память маршрутизатора, количество мест в зале ожидания или ёмкость call-центра. Модель **M/M/m/B** учитывает это.
...
!!! warning "Всегда стабильна!"
    M/M/m/B **всегда** имеет стационарное распределение, даже если $\rho \geq 1$. Причина проста: число заявок ограничено сверху числом $B$, поэтому очередь не может расти бесконечно. Конечно, при $\rho \geq 1$ потери будут огромными, но формально система стабильна.
...
$$
p_n = \begin{cases}
\dfrac{a^n}{n!}\,p_0, & 0 \leq n \leq m \\[10pt]
\dfrac{a^n}{m!\,m^{n-m}}\,p_0, & m < n \leq B
\end{cases}
$$

где $a = \lambda/\mu$, а $p_0$ находится из нормировки $\sum_{n=0}^{B} p_n = 1$.
...
| **M/G/$\infty$** | Бесконечное число серверов (каждый клиент сразу обслуживается) | Нет ожидания. Число заявок в системе — пуассоновское с параметром $\lambda/\mu$. Модель «чистой задержки» |
...
!!! tip "Формула Поллачека--Хинчина — жемчужина теории"
    Для M/G/1 среднее число заявок в очереди зависит только от **первых двух моментов** распределения обслуживания (среднее и дисперсия). Это значит, что детали распределения не так важны — важны среднее и вариабельность. Экспоненциальное распределение ($C_s = 1$) — это «средний» случай; детерминированное ($C_s = 0$) — лучший; а гиперэкспоненциальное ($C_s > 1$) — хуже.

### Как выбрать модель?

```mermaid
flowchart TD
    A["Известны параметры системы"] --> B{"Вход пуассоновский?"}
    B -->|Да| C{"Обслуживание экспоненциальное?"}
    B -->|Нет| D{"Обслуживание экспоненциальное?"}
    C -->|Да| E{"Сколько серверов?"}
    C -->|Нет| F["M/G/1\n(Поллачек—Хинчин)"]
    D -->|Да| G["G/M/1"]
    D -->|Нет| H["G/G/m\n(приближения или симуляция)"]
    E -->|1| I["M/M/1"]
...
Давай подведём итоги того, что мы сегодня разобрали:

1. **Процесс рождения-гибели** — общая рамка для анализа очередей через стационарные вероятности.
2. **M/M/1** — базовая модель с 26 аналитическими результатами. Запомни главное: $E[n] = \rho/(1-\rho)$, и при $\rho \to 1$ всё взрывается.
...
!!! success "Главный вывод"
    Аналитические модели очередей — мощный инструмент для **быстрой оценки** производительности систем. Они не заменяют симуляцию для сложных случаев, но дают отличную интуицию и первое приближение. Если твоя система хоть как-то похожа на M/M/1 — начни с формулы, а уже потом думай, нужна ли симуляция.
