...
Привет! Мы уже разобрались с отдельными системами массового обслуживания (СМО) — одна очередь, один или несколько серверов. Но реальные системы редко устроены так просто. В этой лекции мы поговорим о **сетях массового обслуживания** — когда несколько СМО соединены друг с другом, и заявки путешествуют между ними.

---

## Зачем это нужно

Представь себе любую реальную систему:

- **Компьютерная сеть:** запрос пользователя сначала попадает на веб-сервер, потом на сервер приложений, потом в базу данных, потом обратно — и на каждом этапе есть очередь и обслуживание.
...
Моделировать каждый узел по отдельности бессмысленно — нам важно понимать **взаимодействие** между ними. Именно для этого и нужны сети массового обслуживания.
...
**Открытая сеть** (Open Network) — это сеть, в которую заявки **приходят извне** и **уходят наружу**.
...
!!! info "Как определить пропускную способность?"
    В замкнутой сети нет «входа» и «выхода» в привычном смысле. Пропускная способность определяется как интенсивность потока через любую выбранную связь. Обычно выбирают связь, которая замыкает цикл (OUT $\to$ IN), и измеряют поток через неё.
...
```mermaid
flowchart LR
    EXT["Внешний мир"] -->|"Класс A (открытый)"| S["Сервер"]
    S -->|"Класс A"| OUT["Внешний мир"]
    T1["Терминал 1"] -->|"Класс B (замкнутый)"| S
    S -->|"Класс B"| T1
...
**Последовательная сеть** (Series Network, Tandem Network) — это простейший случай: $k$ узлов типа M/M/1, соединённых последовательно. Выход одного узла — вход следующего.
...
где $\rho_i = \lambda / \mu_i$ — загрузка $i$-го узла.
...
!!! tip "Почему это так важно?"
    Продуктовая форма означает, что **узлы ведут себя «почти» независимо**. Это колоссальное упрощение! Вместо того чтобы анализировать экспоненциально большое пространство состояний всей сети, мы анализируем каждый узел по отдельности.
...
**Модель ремонтника** (Machine Repairman Model) — классическая замкнутая сеть с двумя узлами.

```mermaid
flowchart LR
    W["Работающие машины\n(delay center, N серверов)"] -->|"поломка\n(rate λ)"| R["Ремонтная мастерская\n(m ремонтников)"]
    R -->|"починка\n(rate μ)"| W
